...
def compute_log_p_x(model, x_mb):
    ...
    # comp = torch.distributions.Normal(torch.tensor([-500, -250, 0, 250, 500]), torch.ones(5,)*1000000)
    #
    # gmm = torch.distributions.mixture_same_family.MixtureSameFamily(mix, comp)
    # log_p_y_mb = gmm.log_prob(y_mb).sum(-1)

    log_p_y_mb = torch.distributions.Normal(torch.zeros_like(y_mb), torch.ones_like(y_mb)*1e16).log_prob(y_mb).sum(-1)
...
def main():
    ...
    np.savetxt('lognormal-100-layer1-nodes-8-trdata10k-var1e16-f64.txt', results.detach().cpu().numpy(),  fmt='%20.64f')
